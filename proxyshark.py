...
class DissectedProto(list):
	...
	def __init__(self, etree_proto):
		...
		if not attr_showname:
			raise ValueError("unable to find the protocol description")
...
class Dissector:
	...
	def __init__(self):
		...
		# use tshark from the ./wireshark directory
		tshark_path = os.path.join(os.getcwd(), "wireshark/tshark")
...
class NFQueue(Thread):
	...
	def run(self):
		...
		#
	def set_bpf_filter(self, iface, bpf_filter):
		""" """
		print iface, bpf_filter
		#
	def stop(self):
...
class ThreadingWebServer(ThreadingMixIn, HTTPServer):
	""" Web server with multithreading support for incoming connections. """
	pass
...
class WebServer(Thread):
	...
	def __init__(self, bind_address, port):
		...
		Creates a new WebServer instance.
		  ...
		  bind_adress -- bind address for the local Web server
		  port        -- local port for client connections
		"""
	...
	def run(self):
		...
		self.__server = ThreadingWebServer(
			(self.__bind_address, self.__port),
			WebRequestHandler
		)
		logging.info(
...
class WebRequestHandler(BaseHTTPRequestHandler):
	...
	def handler_request(self):
		...
		# get the path and parameters from the GET request
		findall = re.findall("^/+([^?]*)(\?.*)?$", self.path)
		if not findall:
			self.send_not_found()
			return
		# path
		...
		# parameters
		self.params = {}
		findall = re.findall("([a-z_]+)=([^&]+)", findall[0][1])
		if findall:
		...
		if self.is_ajax():
			...
			elif self.path == "get_network_interfaces":
				self.send_network_interfaces()
			# /set_bpf_filter
			elif self.path == "set_bpf_filter":
				self.send_not_found()
			# /capture_start
			elif self.path == "capture_start":
				self.recv_capture_start()
			# /capture_pause
			elif self.path == "capture_pause":
				self.send_not_found()
			# /capture_stop
			elif self.path == "capture_stop":
				self.send_not_found()
			# /get_packets
			elif self.path == "get_packets":
				self.send_packet_descriptions(None)
			# /get_full_packet
			elif self.path == "get_full_packet":
				self.send_not_found()
			else:
				self.send_not_found()
		# file request
		...
		#
	def is_ajax(self):
		""" Returns True if the current request was made with AJAX. """
	...
	def log_request(self, code):
		...
		#
	def recv_capture_start(self):
		""" """
		json_response = []
		json_network_interface = {'name': name, 'value': value}
		json_response.append(json_network_interface)
		# send the packets
		self.send_response(200, "OK")
		self.send_header("Content-type", "text/plain")
		self.end_headers()
		self.wfile.write(json.dumps(json_response, separators=(",",":")))
		#
	...
	def send_network_interfaces(self):
		""" """
		json_response = []
		tcpdump = Popen("tcpdump -D".split(" "), stdout=PIPE)
		tcpdump_stdout = tcpdump.stdout.read().split("\n")
		for line in tcpdump_stdout:
			value, _, descr = line.partition(".")[2].partition(" ")
			name = "%s %s" % (value, descr)
			json_network_interface = {'name': name, 'value': value}
			json_response.append(json_network_interface)
		...
		self.wfile.write(json.dumps(json_response, separators=(",",":")))
		#
	def send_packet_descriptions(self, min_identifier):
		""" """
		json_response = []
		for packet in [1]:
			json_packet = {}
			# id
			WebRequestHandler.iii += 1
			if WebRequestHandler.iii % 2 == 0:
				self.send_not_found()
				return
			json_packet["identifier"] = WebRequestHandler.iii
			# arrival time
			json_packet["timestamp"] = "123456789"
			# source
			json_packet["source"] = "192.168.1.1"
			# destination
			json_packet["destination"] = "192.168.1.10"
			# port numbers
			json_packet["source_port"] = "12345"
			json_packet["destination_port"] = "80"
			# protocol
			json_packet["protocol"] = "HTTP"
			# info
			json_packet["info"] = "GET / HTTP/1.1"
			json_response.append(json_packet)
		# send the packets
		self.send_response(200, "OK")
		self.send_header("Content-type", "text/plain")
		self.end_headers()
...
def print_usage():
	sys.stderr.write(
"""Usage: %s [-v] [-q <queue_num>] [-a <bind_address>] [-p <port>]

    -v                : verbose mode, can be specified twice for debugging mode
    -q <queue_num>    : specify which netfilter queue must be used
    -a <bind_address> : bind address for the local web server
    -p <port>         : local port for client connections
...
if __name__ == "__main__":
	...
	# parse the arguments
	try:
		opts, args = getopt.getopt(sys.argv[1:], "vq:a:p:")
	...
	try:
		...
		nfqueue.start()
		web_server = WebServer(arg_bind_address, arg_port)
		web_server.start()
	except Exception, exception:
		...
		try:
			...
			while nfqueue.isAlive() and web_server.isAlive():
				time.sleep(0.5)
